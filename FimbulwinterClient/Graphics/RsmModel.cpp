...
bool RsmMesh::Load(RsmHeader &header, const YA3DE::FileSystem::FilePtr &stream)
{
#define IsCompatibleWith(major, minor) (header.Version.Major > major || (header.Version.Major == major && header.Version.Minor >= minor))
	...
	for (int i = 0; i < textureCount; i++)
	{
		...
		_Textures[i] = _Owner->GetTextures()[tex];
	}
...
bool RsmModel::Load(YA3DE::FileSystem::FilePtr stream)
{
#define IsCompatibleWith(major, minor) (header.Version.Major > major || (header.Version.Major == major && header.Version.Minor >= minor))
	RsmHeader header;
...

	{
		...
		for (it = _Meshes.begin(); it != _Meshes.end(); it++)
		{
			...
			it->second->Parent->GetChildren().push_back(it->second);
		}
	}
