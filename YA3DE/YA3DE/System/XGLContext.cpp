...
__thread GLEWContext YA3DE_CurrentGlew;
__thread GLXEWContext YA3DE_CurrentXGlew;

GLContext::GLContext(RenderWindow *owner)
{
	_Window = owner;
	_DeviceContext = owner->DeviceContext();
	_NativeHandle = NULL;
}
...
{
	glXMakeCurrent(_DeviceContext, None, NULL);
}

void GLContext::MakeCurrent()
{
	if (_DeviceContext != NULL && _NativeHandle != NULL)
		glXMakeCurrent(_DeviceContext, _Window->NativeHandle(), _NativeHandle);
}

void GLContext::Destroy()
{
	if (_NativeHandle != NULL)
		glXDestroyContext(_DeviceContext, _NativeHandle);
}
