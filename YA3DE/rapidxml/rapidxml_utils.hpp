...
std::shared_ptr<rapidxml::xml_document<>> YA3DE::Content::ContentManager::LoadNew(const std::string &filename)
{
	...
	char *xml = new char[file->GetSize()];
	file->Read(xml, file->GetSize());
	file->Close();

	std::shared_ptr<rapidxml::xml_document<>> ptr(new rapidxml::xml_document<>());
	ptr->parse<rapidxml::parse_delete_on_destruct>(xml);
