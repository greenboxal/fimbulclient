...
#define IsCompatibleWith(major, minor) (header.Version.Major > major || (header.Version.Major == major && header.Version.Minor >= minor))
	...
	float x = std::cos(glm::radians((float)_Light.Longitude)) * std::cos(glm::radians((float)_Light.Latitude));
	float y = std::cos(glm::radians((float)_Light.Longitude)) * std::sin(glm::radians((float)_Light.Latitude));
	float z = std::sin(glm::radians((float)_Light.Latitude));

	_Light.Position = glm::vec3(-x, y, z);
