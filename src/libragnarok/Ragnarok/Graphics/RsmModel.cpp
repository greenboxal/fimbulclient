...
#include <Ragnarok/Graphics/RsmModel.h>
#include <Ragnarok/Graphics/VertexDeclarations.h>
...
#define IsCompatibleWith(major, minor) (header.Version.Major > major || (header.Version.Major == major && header.Version.Minor >= minor))
	...
	std::vector<RsmTexCoord> texcoords;
	texcoords.resize(texcoordCount);
	for (int i = 0; i < texcoordCount; i++)
	{
		if (IsCompatibleWith(1, 2))
		{
			stream->Read(&texcoords[i].Color, 4 * sizeof(unsigned char));
		}
		else
		{
			texcoords[i].Color.R = 255;
			texcoords[i].Color.G = 255;
			texcoords[i].Color.B = 255;
			texcoords[i].Color.A = 255;
		}

		stream->Read(&texcoords[i].UV, sizeof(glm::vec2));
	}
	...
	{
		RsmFace &face = faces[i];

		stream->Read(&face.Vertices, 3 * sizeof(unsigned short));
		stream->Read(&face.TexCoords, 3 * sizeof(unsigned short));
		stream->Read(&face.TextureID, sizeof(unsigned short));
		stream->Read(&face.Unknown, sizeof(unsigned short));
		stream->Read(&face.TwoSide, sizeof(unsigned int));

		if (IsCompatibleWith(1, 2))
			stream->Read(&face.SmoothGroup, sizeof(unsigned int));
		else
			face.SmoothGroup = 0;

		faceNormals[i] = glm::normalize(glm::cross(_TmpVertex[face.Vertices[2]] - _TmpVertex[face.Vertices[0]], _TmpVertex[face.Vertices[1]] - _TmpVertex[face.Vertices[0]]));
	}
	...
	std::vector<int> shadeGroups[32];

	vectorNormals.resize(triangleCount * 3);

	for (int i = 0; i < triangleCount; i++)
		shadeGroups[faces[i].SmoothGroup].push_back(i);

	std::vector<glm::vec3> tmpNormals;
	tmpNormals.resize(vertexCount);
	for (int i = 0; i < 32; i++)
	{
		for (int n = 0; n < vertexCount; n++)
		{
			...
			{
				RsmFace &face = faces[j];

				if (face.SmoothGroup != i)
					continue;

				if (face.Vertices[0] == n || face.Vertices[1] == n || face.Vertices[2] == n)
					normal += faceNormals[j];
			}

			if (normal.x != 0.f || normal.y != 0.f || normal.z != 0.f)
				tmpNormals[n] = glm::normalize(normal);
		}

		for (int n = 0; n < shadeGroups[i].size(); n++)
		{
			RsmFace &face = faces[n];
			vectorNormals[n * 3 + 0] = tmpNormals[face.Vertices[0]];
			vectorNormals[n * 3 + 1] = tmpNormals[face.Vertices[1]];
			vectorNormals[n * 3 + 2] = tmpNormals[face.Vertices[2]];
		}
	}
	...
	for (int i = 0; i < triangleCount; i++)
	{
		...
		vertices[start + 0] = VertexPositionTextureNormal(_TmpVertex[face.Vertices[0]], vectorNormals[start + 0], texcoords[face.TexCoords[0]].UV);
		vertices[start + 1] = VertexPositionTextureNormal(_TmpVertex[face.Vertices[1]], vectorNormals[start + 1], texcoords[face.TexCoords[1]].UV);
		vertices[start + 2] = VertexPositionTextureNormal(_TmpVertex[face.Vertices[2]], vectorNormals[start + 2], texcoords[face.TexCoords[2]].UV);
