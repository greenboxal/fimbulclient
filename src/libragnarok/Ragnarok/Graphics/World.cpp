...
#include <Ragnarok/Graphics/WorldModel.h>
...
bool World::LoadAltitude(YADE::FilePtr stream)
{
	GatHeader header;
	ContentManager *cm = ContentManager::Instance();
...
bool World::LoadWorld(YADE::FilePtr stream)
{
#define IsCompatibleWith(major, minor) (header.Version.Major > major || (header.Version.Major == major && header.Version.Minor >= minor))
	...
	{
		stream->Read(&_Light.Longitude, sizeof(int));
		stream->Read(&_Light.Latitude, sizeof(int));
		stream->Read(&_Light.Diffuse, sizeof(glm::vec3));
		stream->Read(&_Light.Ambient, sizeof(glm::vec3));
	}
	else
	{
		_Light.Longitude = 45;
		_Light.Latitude = 45;
		_Light.Diffuse = glm::vec3(1.f, 1.f, 1.f);
		_Light.Ambient = glm::vec3(0.3f, 0.3f, 0.3f);
	}

	if (IsCompatibleWith(1, 7))
	{
		stream->Read(&_Light.Intensity, sizeof(float));
	}
	else
	{
	...
	if (_Light.Ambient.x > 1.0f)
		_Light.Ambient.x = 1.0f;

	if (_Light.Ambient.y > 1.0f)
		_Light.Ambient.y = 1.0f;

	if (_Light.Ambient.z > 1.0f)
		_Light.Ambient.z = 1.0f;

	glm::mat4 lightMatrix = glm::rotate(glm::mat4(), (float)_Light.Latitude, glm::vec3(1.f, 0.f, 0.f));
	lightMatrix = glm::rotate(lightMatrix, (float)_Light.Longitude, glm::vec3(0.f, 1.f, 0.f));
	_Light.Position = glm::vec3(lightMatrix[1][0] + lightMatrix[3][0], lightMatrix[1][1] + lightMatrix[3][1], lightMatrix[1][2] + lightMatrix[3][2]);
...
void World::Update(double elapsed)
{
	...
	for (size_t i = 0; i < _Models.size(); i++)
		_Models[i]->Update(SceneCamera(), elapsed);
...
void World::RenderStaticGeometry(double elapsed)
{
	...
	/* Ground */
	{
		...
		_GroundShader->SetUniform("LightIntensity", _Light.Intensity);
		_GroundShader->SetUniform("AmbientLight", _Light.Ambient);
		_GroundShader->SetUniform("DiffuseLight", _Light.Diffuse);
		_Lightmap->Bind(1);
		_GroundVBuffer->Bind();
		for (unsigned int i = 0; i < _GroundTextures.size(); i++)
		{
	...
	_GroundShader->SetUniform("DiffuseLight", _Light.Diffuse);
	for (size_t i = 0; i < _Models.size(); i++)
		_Models[i]->Render(SceneCamera(), elapsed);
	_CommonShader->End();
	glDisable(GL_BLEND);
