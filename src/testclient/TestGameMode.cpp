...
{	
	...
	RagnarokInstance->Gui()->SetDesktop("testmode");

	_World = ContentManager::Instance()->Load<World>("data/" + RagnarokInstance->Settings().get<std::string>("config.ragnarok.startmap", "prontera") + ".rsw");
...
void TestSubGameMode::Update(double elapsed)
{
	...
	if (RagnarokInstance->IsActive())
	{
		...
		bool rightIsPressed = sf::Mouse::isButtonPressed(sf::Mouse::Left) && RagnarokInstance->Gui()->TestPixel(mousePos.x - RagnarokInstance->Window().getPosition().x, mousePos.y - RagnarokInstance->Window().getPosition().y);

		if (rightIsPressed && _RightWasPressed)
        {
			sf::Vector2i center = RagnarokInstance->Window().getPosition() + sf::Vector2i(RagnarokInstance->Window().getSize().x / 2, RagnarokInstance->Window().getSize().y / 2);
