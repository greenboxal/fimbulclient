...
FpsCamera::FpsCamera(const glm::vec3 &position, const glm::vec3 &target, float nearPlane, float farPlane)
{
	...
	_Projection = glm::perspectiveFov<float>(45.f, (float)Application::Instance()->Window().getSize().x, (float)Application::Instance()->Window().getSize().y, nearPlane, farPlane);
	_Dirty = true;
}
...
void FpsCamera::Update(double elapsed)
{
	if (_Dirty)
	{
		...
		_Dirty = false;
		ViewFrustum().Update(_View, _Projection);
	}
}
